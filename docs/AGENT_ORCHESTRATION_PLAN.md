...
**Remaining debt (2026-07-23, narrowed):** Add a concrete IndexedDB cache adapter and Tauri command bridge around the proven Rust wire types; replace or remove the remaining downstream legacy cloud/peer fixtures and expose an authenticated pairing path for non-loopback Mobile access; then run cross-host lifecycle, cancellation, reconnect, and offline-cache acceptance. These integrations remain incomplete, so this section is not marked complete.
