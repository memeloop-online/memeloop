...
**Remaining debt (2026-07-23, narrowed):** The image is buildable and has now passed real local Swarm and K3s execution, but is not yet published to a registry. This minimal image intentionally cannot run profile/model/host-authority workloads until the authenticated worker bootstrap/ModelGateway transport exists. Registry publication and that authenticated bootstrap are required before this section can be marked complete. The conformance, durable result, and real-backend acceptance gaps are cleared.
