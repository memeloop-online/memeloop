...
**2026-07-27 production Audit/Telemetry persistence and routing:** Added `createControlStoreAuditTelemetryAdapter` and the immutable `audit.memeloop.io/v1alpha1 AuditRecord` resource. A named-stream ControlStore lease serializes appenders across daemon instances; every record persists its continuous sequence, preceding digest, full-record SHA-256 digest, exact resource UID, trusted actor/receiver time, capability digest, policy/effect/provenance, bounded attributes, request fingerprint, idempotency digest, and fence. Reads reconstruct and verify the entire ordered chain before returning only the requested resource's records. Stable idempotency names adopt acknowledged writes after recreation, while drift, stale fences, quota overflow, foreign resources, capability/session mismatch, unknown fields, secret-shaped metadata, and broken chain evidence fail closed. `createAuditRecordAuthorizer` runs in the same SQLite/etcd write transaction and makes records create-only; apply, status mutation, deletion, and resource leasing are forbidden. The authorizer is installed by both NodeRuntime's owned SQLite store and the CLI etcd route; injected stores must install an equivalent boundary.

NodeRuntime owns a daemon-random audit capability/session and exposes `managedAuditTelemetryDriver`. The real ModelGateway recorder appends a metadata-only terminal model audit after persisting `ModelCallRecord`; the managed tool executor awaits a non-fatal audit append for success, denial, cancellation, and failure; and WorkerProtocol records both verified requests and unknown-session denials, hashing untrusted targets and session identifiers rather than storing them. Model prompts/completions, tool arguments/results, worker payloads, credentials, and opaque capabilities never enter `AuditRecord`. Event, metric, and trace methods remain available to embedders through the same managed driver.

Focused evidence covers the complete conformance suite on a restart-persistent ControlStore, immutable authorization, capability rejection, per-resource isolation/quota/fencing/idempotency, and two simultaneous adapter instances producing a continuous 12-record chain. Real SQLite NodeRuntime tests prove two model calls, a tool execution, two accepted external-worker requests, and one unknown-session denial create durable records, while asserting prompt and tool payload text are absent. Full validation: core 1047/1047; CLI 416 passed with 8 explicit environment/integration skips; Worker Runtime 7/7, Kubernetes 30/30, Swarm 22/22, libp2p 23/23 plus browser bundle, protocol 6/6, and React UI 33/33; every production/declaration build including Node 24 CLI; and portable boundaries pass. Full lint has zero errors; only four unrelated pre-existing unused-disable warnings remain. This closes the final §24.61 managed-interface production route.
