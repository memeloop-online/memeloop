...
**Remaining debt (2026-07-22):** No Agent-facing deployment API surface yet (a script cannot invoke `deployGeneratedScript` through a ctx helper or tool), and no scheduler/controller consumes `RemoteDeploymentRequest` to bind a node, expose readiness, or honor deletion — that consumption is Phase 4/7 runtime work. Correction of the 2026-07-21 note: `scriptLoader.ts` no longer has an ungated `data:` URL fallback — every source-bearing reference (source refs and `data:` URLs) passes the host-injected `ScriptLoadGate` and fails closed when no gate is configured; the CLI wires the production gate per 24.15.
