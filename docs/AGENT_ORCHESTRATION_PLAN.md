...
**Remaining debt (quality audit):** Phase 7 also requires `ToolOperation`, `ModelEndpoint`, `NetworkAttachment`, and volume placement to be independently reconciled rather than inferred solely from the parent workload. Some selectors/drivers and the volume replication controller exist, but there is no unified independent binding controller or live multi-node capability inventory feeding all of them. This step stays in progress until that control path and its binding-race/fencing tests exist.
