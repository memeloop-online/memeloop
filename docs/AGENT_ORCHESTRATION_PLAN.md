...
**Remaining debt (quality audit, narrowed):** Independent ToolOperation placement and active cancellation/timeout are now real. `ModelEndpoint`, `NetworkAttachment`, credential/volume claims, and volume placement still need a unified live multi-node inventory and independently fenced binding/reconciliation path (the model selector and volume replication controller alone do not prove the Phase 7 requirement).
