...
Scripts execute in a separate VM realm with string/Wasm code generation disabled, no `process` or `fetch`, and no module imports. The container image also runs as the unprivileged `node` user; the drivers add read-only-root, no-new-privileges/capability-drop, and service-account hardening.
