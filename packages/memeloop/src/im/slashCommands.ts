...
export async function tryHandleImSlashCommand(context: ImSlashCommandContext): Promise<{
  ...
  if (cmd === 'switch') {
    ...
    if (!Number.isNaN(n) && n >= 1 && n <= list.length) {
      ...
      targetId = list.find((c) => c.conversationId === argument || c.conversationId.includes(argument))?.conversationId;
    }
  ...
  if (cmd === 'new' || cmd === 'agent') {
    const defId = rest || context.defaultDefinitionId;
    const { conversationId } = await context.driver.createAgent({ definitionId: defId });
