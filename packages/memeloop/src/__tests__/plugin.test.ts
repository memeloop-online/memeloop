  ...
  it('accepts a valid manifest and normalizes optional fields', () => {
    expect(
      validatePluginManifest({
        ...
        exports: { tools: ['tool-a', 1], hooks: ['PreToolUse', 'Nope'], skills: ['skill-a'] },
      }),
    ).toEqual({
      name: 'plugin-a',
      version: '1.0.0',
      ...
      author: 'Alice',
      minMemeloopVersion: '0.1.0',
      exports: { tools: ['tool-a'], hooks: ['PreToolUse'], skills: ['skill-a'] },
    });
  });
...
describe('loadPluginModule', () => {
  it('loads a plugin and calls activate', async () => {
    const activate = vi.fn((api: unknown) => {
      (api as { registerTool(id: string, impl: () => string): void }).registerTool('test.tool', () => 'ok');
    });
  ...
  it('returns existing loaded plugin if already loaded', async () => {
    const first = await loadPluginModule({ manifest: manifest({ name: 'dup-plugin' }), module: module({ name: 'dup-plugin' }) });
    const second = await loadPluginModule({ manifest: manifest({ name: 'dup-plugin' }), module: module({ name: 'dup-plugin' }) });
    expect(second).toBe(first);
  });

  it('returns null for invalid manifests or modules', async () => {
    expect(await loadPluginModule({ manifest: manifest({ name: '' }), module: module() })).toBeNull();
    expect(
...
describe('registerPluginTools/Hooks/Skills tracking', () => {
  ...
  it('tracks registered skills and clears all tracking', () => {
    registerPluginSkills('test-plugin', [
      { id: 'skill-a', name: 'A', instructions: 'do a' },
      { id: 'skill-b', name: 'B', instructions: 'do b' },
    ]);
    expect(getPluginRegistrations('test-plugin')?.skills).toEqual(['skill-a', 'skill-b']);
