...
import { autoCompact, type CompactionOptions, compactMessages, shouldCompact } from '../agent-tool-loop/compaction.js';
    ...
    if (withTools && t % 2 === 0) {
      messages.push(
        createMessage({
          id: base + 3,
          role: 'tool',
          content: `Tool output for turn ${t + 1}. This is a very long tool output that contains a lot of data and should be summarized during compaction. `.repeat(
            5,
          ),
        }),
      );
