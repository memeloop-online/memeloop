...
import { autoCompact, type CompactionOptions, compactMessages, shouldCompact } from '../compact.js';
...
function createConversation(
  ...
  for (let t = 0; t < turnCount; t++) {
    const base = t * 3;
    messages.push(
      createMessage({ id: base + 1, role: 'user', content: `User message ${t + 1}` }),
    );
    messages.push(
      createMessage({
        id: base + 2,
    ...
    if (withTools && t % 2 === 0) {
      messages.push(
        createMessage({
          id: base + 3,
          role: 'tool',
          content: `Tool output for turn ${t + 1}. This is a very long tool output that contains a lot of data and should be summarized during compaction. `.repeat(5),
        }),
      );
    }
  }
...
describe('shouldCompact', () => {
  ...
  it('returns true when above threshold', () => {
    const messages = new Array(60).fill(null).map((_, i) => createMessage({ id: i, role: 'user', content: `Msg ${i}` }));
    expect(shouldCompact(messages, 50)).toBe(true);
  });

  it('uses default threshold of 50', () => {
    const messages = new Array(51).fill(null).map((_, i) => createMessage({ id: i, role: 'user', content: `Msg ${i}` }));
    expect(shouldCompact(messages)).toBe(true);
  });

  it('returns false when at exact threshold', () => {
    const messages = new Array(50).fill(null).map((_, i) => createMessage({ id: i, role: 'user', content: `Msg ${i}` }));
    expect(shouldCompact(messages, 50)).toBe(false);
  });
});
