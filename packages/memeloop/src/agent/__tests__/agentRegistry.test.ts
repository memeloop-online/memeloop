...
import { AgentRegistry, getAgentRegistry, resetAgentRegistry } from '../agentRegistry.js';
import type { AgentRegistryEntry } from '../agentTypes.js';
import { buildAgent, exploreAgent, librarianAgent, oracleAgent, planAgent, PREDEFINED_AGENTS } from '../agentTypes.js';
...
describe('AgentRegistry', () => {
  ...
  it('throws when registering an agent with empty id', () => {
    expect(() => {
      registry.registerAgent(makeTestDef({ id: '' }));
    }).toThrow(
      /non-empty id/,
    );
  });
