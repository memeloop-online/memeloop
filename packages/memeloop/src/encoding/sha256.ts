...
/** Small portable synchronous SHA-256 used for already-bounded runtime principals. */
export function sha256HexSync(bytes: Uint8Array): string {
  ...
  for (let offset = 0; offset < paddedLength; offset += 64) {
    ...
    let [a, b, c, d, stateE, f, g, h] = hash;
    for (let index = 0; index < 64; index += 1) {
      const upperSigma1 = rotateRight(stateE, 6) ^ rotateRight(stateE, 11) ^
        rotateRight(stateE, 25);
      const choice = (stateE & f) ^ (~stateE & g);
