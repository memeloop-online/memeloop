...
import { loadPresets, findPreset, type PresetProvider } from "./presets.js";
import {
  listProviders,
  addProvider,
  removeProvider,
...
import { setApiKey, getAuthPath } from "../auth/authStore.js";
...
function detectClaudeCode(): DetectedToolConfig | null {
  ...
  try {
    ...
    const lm = data.lm;
    if (lm) {
      for (const [key, val] of Object.entries(lm) as Array<[string, any]>) {
        if (val && typeof val === "object" && val.provider) {
          providers.push({
            name: key,
            baseUrl: val.baseUrl || val.baseURL,
            apiKey: val.apiKey,
          });
        }
      }
    ...
    if (data && typeof data === "object") {
      for (const [key, val] of Object.entries(data) as Array<[string, any]>) {
        if (val && typeof val === "object" && val.key) {
          providers.push({ name: key, apiKey: val.key });
        }
      }
    }
...
export function ConfigTUI() {
  ...
  useInput(
    useCallback(
      ...
      [view, selectedIndex, providers, presets, presetIndex, addName, addBaseUrl, addApiKey, addFieldIndex, editState, importText, detectedTools, migrationSelected, cloudUrl, cloudOtp, cloudFieldIndex, cloudRegistering],
    ),
  );
  ...
  function handleMainInput(input: string, key: any) {
    if (key.upArrow) {
      ...
      setSelectedIndex((i) => Math.min(providers.length, i + 1)); // +1 for "Add" option
  ...
  function handleAddPresetInput(input: string, key: any) {
    if (key.upArrow) {
      ...
      setPresetIndex((i) => Math.min(presets.length - 1, i + 1));
    } else if (key.return) {
  ...
  function handleAddManualInput(input: string, key: any) {
    ...
    if (key.tab) {
      setAddFieldIndex((i) => (i + 1) % 3);
      return;
    }
    if (key.upArrow) {
      setAddFieldIndex((i) => (i === 0 ? 2 : i - 1));
      return;
    }
    if (key.downArrow || key.return) {
      if (addFieldIndex < 2) {
        setAddFieldIndex((i) => i + 1);
      } else {
        // Submit
        if (!addName.trim()) {
  ...
  function handleEditInput(input: string, key: any) {
    ...
    if (key.tab || key.downArrow) {
      setEditState((s) => s && { ...s, fieldIndex: ((s.fieldIndex + 1) % 3) as EditState["fieldIndex"] });
      return;
    }
    if (key.upArrow) {
      setEditState((s) => s && { ...s, fieldIndex: (s.fieldIndex === 0 ? 2 : s.fieldIndex - 1) as EditState["fieldIndex"] });
      return;
    }
    ...
    if (key.backspace || key.delete) {
      setEditState((s) => {
        ...
        return { ...s, [field]: (s[field] as string).slice(0, -1) };
      });
      return;
    }
    if (input && input.length === 1 && !key.ctrl && !key.meta) {
      setEditState((s) => {
        ...
        const fields = ["name", "baseUrl", "apiKey"] as const;
        const field = fields[s.fieldIndex];
        return { ...s, [field]: (s[field] as string) + input };
      });
    }
  }
  ...
  function handleCloudInput(input: string, key: any) {
    ...
    if (key.tab || key.downArrow) {
      setCloudFieldIndex((i) => (i + 1) % 2);
      return;
    }
    if (key.upArrow) {
      setCloudFieldIndex((i) => (i === 0 ? 1 : i - 1));
      return;
    }
    if (key.return) {
      if (cloudFieldIndex === 0) {
        ...
        if (!cloudUrl.trim() || !cloudOtp.trim()) {
          setMessage("Cloud URL and OTP are required. / Cloud URL 和 OTP 不能为空。");
          return;
        }
        ...
        void (async () => {
          try {
            ...
            const { loadConfig: lc, saveConfig: sc, getDefaultConfigPath: gdcp } = await import("../config.js");
            const { loadOrCreateNodeKeypair, getDefaultKeypairPath } = await import("../auth/keypair.js");
            const client = new CloudClient(cloudUrl.trim());
            const keypair = loadOrCreateNodeKeypair(getDefaultKeypairPath());
            const result = await client.registerWithOtp(cloudOtp.trim(), {
              x25519PublicKey: keypair.x25519PublicKey,
              ed25519PublicKey: keypair.ed25519PublicKey,
            });
            const cfg = lc();
            cfg.nodeId = result.nodeId || keypair.nodeId;
            if (result.nodeSecret) cfg.nodeSecret = result.nodeSecret;
            cfg.cloudUrl = cloudUrl.trim();
            sc(cfg);
            setMessage(`Registered! nodeId: ${cfg.nodeId} / 注册成功！`);
          } catch (e: any) {
            setMessage(`Register failed: ${e.message} / 注册失败：${e.message}`);
          } finally {
            setCloudRegistering(false);
          }
        })();
  ...
  function renderMain() {
    ...
    return (
      <Box flexDirection="column" padding={1}>
          ...
          {providers.map((p, i) => (
            <Box key={p.name}>
              <Text color={i === selectedIndex ? "cyan" : undefined}>
                ...
                {p.name.padEnd(20)}
                {p.hasApiKey ? (
                  <Text color="green">{p.apiKeyMasked}</Text>
                ) : (
        ...
        <Box marginTop={1}>
          <Text dimColor>
            <Text color="green">A</Text>dd preset{"  "}
            <Text color="green">M</Text>anual{"  "}
            <Text color="green">E</Text>dit{"  "}
            <Text color="green">D</Text>el{"  "}
            <Text color="green">I</Text>mport{"  "}
            e<Text color="green">X</Text>port{"  "}
            mi<Text color="green">G</Text>rate{"\n"}
            <Text color="green">N</Text>ode{"  "}
            <Text color="green">H</Text>ealth{"  "}
            <Text color="green">C</Text>loud{"  "}
            <Text color="green">Q</Text>uit
          </Text>
        </Box>
      </Box>
    );
  ...
  function renderAddPreset() {
    return (
      <Box flexDirection="column" padding={1}>
        ...
        {presets.map((p, i) => (
          <Box key={p.name}>
            <Text color={i === presetIndex ? "cyan" : undefined}>
              {i === presetIndex ? "▶ " : "  "}
              {p.name.padEnd(20)}
        ...
        <Box marginTop={1}>
          <Text dimColor>
            ...
            <Text color="green">M</Text> manual entry{"  "}
  ...
  function renderAddManual() {
    ...
    return (
      <Box flexDirection="column" padding={1}>
        ...
        {fields.map((label, i) => {
          const value = i === 0 ? addName : i === 1 ? addBaseUrl : i === 2 ? addApiKey : "";
          const displayValue = i === 2 && value ? maskKey(value) : value;
          return (
            <Box key={label}>
              <Text color={i === addFieldIndex ? "green" : undefined}>
                {i === addFieldIndex ? "▶ " : "  "}
                {label.padEnd(12)}: {displayValue}
                {i === addFieldIndex ? <Text color="cyan">█</Text> : null}
              </Text>
            </Box>
          );
        })}
        ...
        <Box marginTop={1}>
          <Text dimColor>
            <Text color="green">Tab/↓↑</Text> switch field{"  "}
  ...
  function renderEdit() {
    ...
    return (
      <Box flexDirection="column" padding={1}>
        ...
        {fields.map((label, i) => {
          const value = i === 0 ? editState.name : i === 1 ? editState.baseUrl : i === 2 ? editState.apiKey : "";
          const displayValue = i === 2 && value ? maskKey(value) : value;
          return (
            <Box key={label}>
              <Text color={i === editState.fieldIndex ? "green" : undefined}>
                {i === editState.fieldIndex ? "▶ " : "  "}
                {label.padEnd(25)}: {displayValue}
                {i === editState.fieldIndex ? <Text color="cyan">█</Text> : null}
              </Text>
            </Box>
          );
        })}
        ...
        <Box marginTop={1}>
          <Text dimColor>
            <Text color="green">Tab/↓↑</Text> switch field{"  "}
            <Text color="green">Enter</Text> save{"  "}
            <Text color="green">Esc</Text> back
          </Text>
        </Box>
      </Box>
  ...
  function renderDeleteConfirm() {
    ...
    return (
      <Box flexDirection="column" padding={1} borderStyle="round" borderColor="red">
        ...
        <Box marginTop={1}>
          <Text>
            <Text color="green">Y</Text> confirm / 确认{"  "}
            <Text color="red">N</Text> cancel / 取消
          </Text>
        </Box>
      </Box>
    );
  ...
  function renderMigration() {
    ...
    return (
      <Box flexDirection="column" padding={1}>
        ...
        <Box marginTop={1}>
          <Text dimColor>
            <Text color="green">Enter</Text> toggle select{"  "}
            <Text color="green">A</Text> select all{"  "}
            <Text color="green">S</Text> import selected{"  "}
            <Text color="green">Esc</Text> back
          </Text>
        </Box>
      </Box>
  ...
  function renderDiagnostics() {
    ...
    const errCount = diagChecks.filter((c) => c.status === "error").length;
    ...
    return (
      <Box flexDirection="column" padding={1}>
        ...
        {diagRunning ? (
          <Text>Running checks... / 运行检查中...</Text>
        ) : (
          <>
            {diagChecks.map((c, i) => {
  ...
  function renderCloud() {
    const fields = ["Cloud URL", "OTP Code"];
    return (
      <Box flexDirection="column" padding={1}>
        <Box marginBottom={1}>
        ...
        {cloudRegistering ? (
          <Text>Registering... / 注册中...</Text>
        ) : (
          <>
            {fields.map((label, i) => {
              const value = i === 0 ? cloudUrl : cloudOtp;
              const displayValue = i === 1 ? "*".repeat(value.length) || "" : value;
              return (
                <Box key={label}>
                  <Text color={i === cloudFieldIndex ? "green" : undefined}>
                    {i === cloudFieldIndex ? "▶ " : "  "}
                    {label.padEnd(14)}: {displayValue}
                    {i === cloudFieldIndex ? <Text color="cyan">█</Text> : null}
                  </Text>
                </Box>
              );
            })}
        ...
        <Box marginTop={1}>
          <Text dimColor>
            <Text color="green">Tab</Text> switch field{"  "}
            <Text color="green">Enter</Text> register{"  "}
            <Text color="green">Esc</Text> back
          </Text>
        </Box>
      </Box>
